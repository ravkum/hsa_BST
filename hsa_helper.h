...
#define ASSERT_CL(STATUS, MSG) if ((STATUS)!=CL_SUCCESS) { if (MSG!=NULL) {printf(MSG); printf("status: %d\n",(STATUS)); printf("\n");} exit(!CL_SUCCESS); } 
...
cl_context createHSAContext(cl_platform_id platform) {
	cl_int status = 0;
...
DeviceSVMMode detectSVM(cl_device_id device_id)
{
	...
	if (CL_SUCCESS == clGetDeviceInfo(device_id, CL_DEVICE_VERSION, 1000, 
		device_version_string, NULL)) {
			...
			if (sscanf_s(device_version_string, "OpenCL %d.%d", &majorRev, &minorRev) == 2) {
				if (majorRev >= 2) {
					deviceSVM = SVM_CL20;
				}
			}
